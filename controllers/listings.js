...
module.exports.renderNewForm = (req, res) => { //isLoggedIn middleware will check if the user is authenticated or not, if not then it will redirect to the login page with a flash message. if the user is authenticated then next middleware will be called which is the route handler for creating a new listing.
...
module.exports.showListing = async (req, res) => {
    ...
    const listing = await Listing.findById(id).populate({path: "reviews", populate: {path: "author",},}).populate("owner") ; // this will fetch the listing by id and populate the reviews field with the associated reviews.
